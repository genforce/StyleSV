...
@click.option('--interpw', help='Print optional information', type=bool, default=False, metavar='BOOL', show_default=True)
...
def calc_metrics(ctx, network_pkl, networks_dir, metrics, data, mirror, gpus, cfg_path, verbose, interpw, use_cache: bool, num_runs: int):
    """Calculate quality metrics for previous training run or pretrained network pickle.
    ...
    # Validate arguments.
    args = dnnlib.EasyDict(metrics=metrics, num_gpus=gpus, network_pkl=network_pkl, verbose=verbose, interpw=interpw)
    if cfg_path == "auto":
