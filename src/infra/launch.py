...
def main(cfg: DictConfig):
    ...
    training_cmd = f'{before_train_cmd}TORCH_EXTENSIONS_DIR={torch_extensions_dir} cd {cfg.project_release_dir} && python src/train.py {HYDRA_ARGS}'
    ...
    if cfg.slurm:
        print('dataset path:', cfg.dataset.path_for_slurm_job)
        if 's3' in cfg.dataset.path:
            cfg.dataset.path = cfg.dataset.path[5:] 
        assert Path(cfg.dataset.path_for_slurm_job).exists() or cfg.dataset.path.startswith('s3')
        ...
        for i in range(cfg.job_sequence_length):
            ...
            submit_job_cmd = f'srun -N1 {cfg.sbatch_args_str} --export=ALL,{cfg.env_args_str} python -u {cfg.project_release_dir}/src/infra/slurm_job.py'
            print('submit command:', submit_job_cmd)
